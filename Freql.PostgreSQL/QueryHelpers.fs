...
module QueryHelpers =
    ...
    let inline mapResult<'T>  (reader: NpgsqlDataReader) ordinal supportType =
        match supportType with
            | SupportedType.Boolean -> reader.GetBoolean(ordinal) :> obj
            | SupportedType.Byte -> reader.GetByte(ordinal) :> obj
            | SupportedType.SByte -> reader.GetByte(ordinal) |> sbyte :> obj // TODO not supported in SQLServer - document this
            ...
            | SupportedType.UInt -> reader.GetInt32(ordinal) |> uint :> obj
            ...
            | SupportedType.Option st ->
                ...
                | false ->
                    ...
                    | SupportedType.UShort -> Some(reader.GetInt16(ordinal) |> uint32) :> obj
                    ...
                    | SupportedType.TimeSpan -> Some(reader.GetTimeSpan(ordinal)) :> obj
    ...
    let mapResults<'T> (mappedObj: MappedObject) (reader: DbDataReader) =
        let getValue (reader: DbDataReader) o supportType =
            match supportType with
    ...
    [<CompilerMessage("This is a new implementation and requires testing/benchmarking. To remove this warning add #nowarn \"6140003\"", 6140003)>]
    let mapSingleResult<'T> (mappedObj: MappedObject) (reader: DbDataReader) =
        let getValue (reader: DbDataReader) o supportType =
            match supportType with
            | SupportedType.Boolean -> reader.GetBoolean(o) :> obj
            | SupportedType.Byte -> reader.GetByte(o) :> obj
    ...
    let deferredMapResults<'T> (mappedObj: MappedObject) (comm: NpgsqlCommand) =
        let getValue (reader: DbDataReader) o supportType =
            match supportType with
            | SupportedType.Boolean -> reader.GetBoolean(o) :> obj
            | SupportedType.Byte -> reader.GetByte(o) :> obj
    ...
    let rawNonQuery (connection: NpgsqlConnection) (sql: string) (transaction: NpgsqlTransaction option) =
    ...
    let create<'T> (tableName: string) (connection: NpgsqlConnection) (transaction: NpgsqlTransaction option) =
    ...
    let selectAll<'T> (tableName: string) (connection: NpgsqlConnection) (transaction: NpgsqlTransaction option) =
        let mappedObj = MappedObject.Create<'T>()
    ...
    let executeScalar<'T> (sql: string) (connection: NpgsqlConnection) (transaction: NpgsqlTransaction option) =
        let comm = noParam connection sql transaction

        comm.ExecuteScalar() :?> 'T

    let selectSql<'T> (sql: string) (connection: NpgsqlConnection) (transaction: NpgsqlTransaction option) =
        let tMappedObj = MappedObject.Create<'T>()
    ...
    let deferredSelectSql<'T> (sql: string) (connection: NpgsqlConnection) (transaction: NpgsqlTransaction option) =
        let tMappedObj = MappedObject.Create<'T>()
    ...
    module private Insert =
        ...
        let handleBlobCallbacks
            ...
            callbacks
            |> List.map (fun cb ->
                use writeStream = new (*NpgsqlLargeObjectManager()*) NpgsqlBlob(connection, tableName, cb.ColumnName, rowId)
